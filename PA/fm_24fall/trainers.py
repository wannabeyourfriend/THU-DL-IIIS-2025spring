...
class SFTTrainer(Trainer):
    ...
    def fit(self):
        ...
        # 更新 GradScaler 初始化
        scaler = torch.amp.GradScaler('cuda', enabled=self.dtype != torch.float32)
...
class DPOTrainer(Trainer):
    ...
    def shared_step(self, completions, attention_masks):
        ...
        ############################ Your code here ############################
        # 获取正样本和负样本
        positive_completions = completions[:, 0]
        positive_attention_masks = attention_masks[:, 0]
        negative_completions = completions[:, 1]
        negative_attention_masks = attention_masks[:, 1]
        
        # 使用当前模型计算正负样本的对数概率
        with torch.no_grad():
            positive_log_probs_sft = self.sft_model.get_log_p(positive_completions, positive_attention_masks)
            negative_log_probs_sft = self.sft_model.get_log_p(negative_completions, negative_attention_masks)
        
        # 使用训练中的模型计算正负样本的对数概率
        positive_log_probs = self.model.get_log_p(positive_completions, positive_attention_masks)
        negative_log_probs = self.model.get_log_p(negative_completions, negative_attention_masks)
        
        # 计算DPO损失和准确率
        loss, acc = self.criterion(positive_log_probs, negative_log_probs, 
                                  positive_log_probs_sft, negative_log_probs_sft)
